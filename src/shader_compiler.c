...
static void split_path(char const* const path, char* const dir, size_t const dir_size,
                       char* const stem, size_t const stem_size)
{
...
static bool compile_shader(char const* const input_file, compiled_shader* const result)
{
    ...
    SDL_ShaderCross_HLSL_Info info = {.source = hlsl,
                                      .entrypoint = "main",
                                      .include_dir = dir,
                                      .defines = 0,
                                      .shader_stage = stage,
    ...
    SDL_ShaderCross_SPIRV_Info const spirv_info = {.bytecode = spirv,
...
#error "Only Apple and Windows platforms are supported."
    ...
    snprintf(command, sizeof(command), "%s \"%s\" -o \"%s\"", metallib_command, air_path,
             metallib_path);
    if (!run_command(command))
    {
