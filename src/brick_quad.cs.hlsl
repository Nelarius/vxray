...
Texture3D<uint> brick_masks : register(t0, space0);
...
{
    // Neighbors beyond the scene are empty.
    if (any(uint3(brick) >= uniforms.brick_grid_ext))
    {
        return false;
    }
    uint3 const local = uint3(brick) & (VX_MASK_EXT - 1u);
    uint const  idx = local.x + local.y * VX_MASK_EXT + local.z * VX_MASK_EXT * VX_MASK_EXT;
    return (brick_masks.Load(int4(brick / VX_MASK_EXT, 0)).r & (1u << idx)) != 0u;
}
