...
bool ray_box_test(
    float3 const ray_origin, float3 const inv_ray_dir, float3 const p0, float3 const p1,
...
uint dda(float3 const origin, float3 const dir)
{
    ...
    float3 const exit = origin + tmax * dir;
    ...
    int3 const   end_cell = clamp(int3(exit), min_cell, max_cell);
    int3 const   ds = abs(end_cell - start_cell);
    int const    max_steps = ds.x + ds.y + ds.z + 1;
...
float4 main(ps_input const input) : SV_Target0
{
    ...
    float2 const ndc = float2(
        (pixel.x / uniforms.viewport.x) * 2.0 - 1.0, 1.0 - (pixel.y / uniforms.viewport.y) * 2.0);
    ...
    float3 const dir = normalize(
        uniforms.camera_forward.xyz + uniforms.camera_right.xyz * ndc.x * aspect * scale +
